...
# Get data
###
hip_trajectory <- read.table("final_result_perturbation_smooth_recovery.txt", header = TRUE)

# 'time' conflicts with a built-in function. rename it
hip_trajectory <- rename(hip_trajectory, c(time = "temps"))
...
# HIPS 
hip.s <- pos_reshape(hip_trajectory, 'H')
p_hip_error <- qplot(temps, Hpos, colour = type, data = hip.s, geom = 'line',
	xlab = "Time", ylab = "Hip Position [rad]", 
	main = "Hip position error") + facet_grid(limb ~ .) + no_legend

# KNEES
# knee.s <- pos_reshape(knee_trajectory.r, 'K')
# p_knee_error <- qplot(temps, Kpos, colour=type, data = knee.s, geom='line',
	# xlab = "Time", ylab = "Knee Position [rad]",
	# main = "Knee position error") + facet_grid(limb ~ .) + no_legend
...
# p_pos_phase <- qplot(Hpos, Kpos, data = real_pos, geom = 'path', colour = temps,
	# xlab = "Hip Position [rad]", ylab = "Knee Position [rad]", 
	# main = "Position Phase Plot") + facet_wrap(~ limb, scales = 'free') +
	# no_legend
