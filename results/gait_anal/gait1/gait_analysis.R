...
sel <- c('distance', grep('^x', names(leg_trajectory_rob.r), value = TRUE))
...
forces.n <- forces
sel <- grep('^fy', names(forces.n), value = TRUE) # keep only y direction
forces.n[sel] <- lapply(forces.n[sel], '>', threshold) # TRUE or FALSE
print(mean(forces.n[sel]))

# do the same for the restricted studied area
forces.n <- forces.r
...
### Specific plots
ggsave("gait1_forces.pdf", plot = p_force, width = 12, height = 7.5)
ggsave("gait1_hips.pdf", plot = p_hip_error, width = 12, height = 7.5)
ggsave("gait1_knees.pdf", plot = p_knee_error, width = 12, height = 7.5)
ggsave("gait1_all.pdf", plot = p_all, width = 12, height = 7.5)
ggsave("gait1_phase.pdf", plot = p_pitch_vs_roll, width = 12, height = 7.5)

pdf("gait1_trajectory.pdf", width = 12, height=7.5)
